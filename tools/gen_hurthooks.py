...
with open("../jni/hurthook.h", "w") as outfile, open("entityvtables.txt", "r") as infile:
	...
	print("#pragma once\n// Autogenerated - do not edit\n" + 
		"typedef int (*bl_entity_hurt_t)(Entity*, EntityDamageSource const&, int);", file=outfile)
	print("static bl_entity_hurt_t " + ",".join(["bl_" + v + "_hurt_real" for v in indata]) + ";", file=outfile)
	for v in indata:
		print("static int bl_" + v + "_hurt_hook(Entity* entity, EntityDamageSource const& damage, int hearts) {\n" + 
			"	return bl_Entity_hurt_report(entity, damage, hearts)? 0: bl_" + v + "_hurt_real(entity, damage, hearts);\n" +
			"}", file=outfile)
